...
async function runTests() {
  ...
  const record3 = otpStore.get(user3)
  if (record3) record3.expiresAt = Date.now() - 1000
  ...
  if (record4) record4.lastSentAt = Date.now() - 31000
  ...
  if (!genA.success) throw new Error('Failed to generate OTP for User A')
  const recordA = otpStore.get(userA)
  if (recordA) recordA.lastSentAt = Date.now() - 31000

  const genB = generateOTP(userB)
  if (!genB.success) throw new Error('Failed to generate OTP for User B')
